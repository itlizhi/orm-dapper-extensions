...
namespace Dapper.Orm
{
    ...
    public static class DapperExtensions
    {
        ...
        public class Config
        {
            /// <summary>
            /// 是否启用伪删除
            /// 设定true后，orm生成的删除语句会转换为update xxx set {FalseDeleteSet}   isdelete=1
            /// 直接写的sql语句不会转换
            /// </summary>
            public static bool IsEnableFalseDelele = false;

            /// <summary>
            /// 伪删除set的值
            /// </summary>
            public static string FalseDeleteSet = " isdelete=1";

            /// <summary>
            /// 是否启用全局过滤器
            /// 设定为true 后，orm生成的sql语句会自动添加 {GlobalFilterWhere} isdelete=1 条件
            /// 直接写的sql语句需要手动添加
            /// </summary>
            public static bool IsEnableGlobalFilter = false;

            /// <summary>
            /// 全局过滤的条件
            /// </summary>
            public static string GlobalFilterWhere = " and {0}.isdelete =0";
        ...
        internal static string AppendFilterWhere(this string sql, Type table)
        {
            var tableName = Resolvers.Table(table);

            if (sql.ToLower().Contains("where"))
            {
                return sql += string.Format(Config.GlobalFilterWhere, tableName);
            }
            else
            {
                return sql += " where 1=1 " + string.Format(Config.GlobalFilterWhere, tableName);
            }
        }
        ...
        private static string BuildGetById(Type type, object id, out DynamicParameters parameters)
        {
            ...
            //全局过滤
            if (Config.IsEnableGlobalFilter)
            {
                sql = sql.AppendFilterWhere(type);
            }
        ...
        private static string BuildMultiMapQuery(Type resultType, Type[] includeTypes, object id, out DynamicParameters parameters)
        {
            ...
            if (Config.IsEnableGlobalFilter)
            {
                sql = sql.AppendFilterWhere(resultType);
            }
        ...
        /// <returns></returns>
        public static IEnumerable<TEntity> In<TEntity>(this IDbConnection connection, IEnumerable<int> array, bool buffered = true)
        {
            if (array == null || array.Count() == 0) { return null; }
            DynamicParameters parameters;
            var filed = Resolvers.KeyProperty(typeof(TEntity)).Name; //获取主键
            var sql = BuildInSql<TEntity>(filed, array, out parameters);
            return connection.Query<TEntity>(sql, parameters, buffered: buffered);
        }
        ...
        /// <param name="buffered"></param>
        /// <returns></returns>
        public static IEnumerable<TEntity> In<TEntity>(this IDbConnection connection, string filed, IEnumerable<int> array, bool buffered = true)
        {
            DynamicParameters parameters;
            var sql = BuildInSql<TEntity>(filed, array, out parameters);
            return connection.Query<TEntity>(sql, parameters, buffered: buffered);
        ...
        /// <param name="buffered"></param>
        /// <returns></returns>
        public static IEnumerable<TEntity> In<TEntity>(this IDbConnection connection, IEnumerable<string> array, bool buffered = true)
        {
            DynamicParameters parameters;
            var filed = Resolvers.KeyProperty(typeof(TEntity)).Name; //获取主键
            var sql = BuildInSql<TEntity>(filed, array, out parameters);
        ...
        /// <param name="buffered"></param>
        /// <returns></returns>
        public static IEnumerable<TEntity> In<TEntity>(this IDbConnection connection, string filed, IEnumerable<string> array, bool buffered = true)
        {
            DynamicParameters parameters;
            var sql = BuildInSql<TEntity>(filed, array, out parameters);
            return connection.Query<TEntity>(sql, parameters, buffered: buffered);
        ...
        private static string BuildInSql<TEntity>(string filed, IEnumerable<int> array, out DynamicParameters parameters)
        {
            var type = typeof(TEntity);
            var tableName = Resolvers.Table(type);
            string sql = $"select * from {tableName} where {filed} in @p0";
            parameters = new DynamicParameters();
            parameters.Add("@p0", array);
        ...
        private static string BuildInSql<TEntity>(string filed, IEnumerable<string> array, out DynamicParameters parameters)
        {
            var type = typeof(TEntity);
            var tableName = Resolvers.Table(type);
            string sql = $"select * from {tableName} where {filed} in @p0";
        ...
        public class SqlExpression<TEntity>
        {
            ...
            /// <returns>The current <see cref="DapperExtensions.SqlExpression&lt;TEntity&gt;"/> instance.</returns>
        ...
        private static string BuildInsertQuery(IDbConnection connection, Type type)
        {
            ...
            if (!_insertQueryCache.TryGetValue(type, out sql))
            {
                ...
                //非自增主键
                if (keyProperty.GetCustomAttributes().Any(a => a is ExplicitKeyAttribute))
                {
                    typeProperties.Insert(0, keyProperty);
                }
        ...
        private static string BuildUpdateQuery(Type type)
        {
            ...
            if (!_updateQueryCache.TryGetValue(type, out sql))
            {
                ...
                var typeProperties = Resolvers.Properties(type).Where(p => p != keyProperty)
                                              .Where(p => p.GetSetMethod() != null)
                                              .ToArray();
        ...
        private static string BuildDeleteQuery(Type type)
        {
            ...
            if (!_deleteQueryCache.TryGetValue(type, out sql))
            {
                ...
                //伪删除
                if (Config.IsEnableFalseDelele)
                {
                    sql = $"update {tableName} set {Config.FalseDeleteSet} where {keyColumnName} = @{keyProperty.Name}";
                }
                else
                {
        ...
        private static string BuildDeleteMultipleQuery<TEntity>(Expression<Func<TEntity, bool>> predicate, out DynamicParameters parameters)
        {
            ...
            if (!_deleteAllQueryCache.TryGetValue(type, out sql))
            {
                ...
                if (Config.IsEnableFalseDelele)
                {
                    sql = $"update {tableName} set {Config.FalseDeleteSet}";
                }
                else
                {
        ...
        public static class Resolvers
        {
            ...
            /// <summary>
            /// Gets the key property for the specified type, using the configured <see cref="DapperExtensions.IKeyPropertyResolver"/>.
            /// </summary>
            ...
            /// <see cref="DapperExtensions.IPropertyResolver"/>.
            /// </summary>
            ...
            /// using the configured <see cref="DapperExtensions.ITableNameResolver"/>.
            /// </summary>
        ...
        /// <summary>
        /// Sets the <see cref="DapperExtensions.IPropertyResolver"/> implementation for resolving key of entities.
        /// </summary>
        /// <param name="propertyResolver">An instance of <see cref="DapperExtensions.IPropertyResolver"/>.</param>
        ...
        /// <summary>
        /// Default implemenation of the <see cref="DapperExtensions.IPropertyResolver"/> interface.
        /// </summary>
        ...
        /// <summary>
        /// Sets the <see cref="DapperExtensions.IKeyPropertyResolver"/> implementation for resolving key properties of entities.
        /// </summary>
        /// <param name="resolver">An instance of <see cref="DapperExtensions.IKeyPropertyResolver"/>.</param>
        ...
        /// <summary>
        /// Implements the <see cref="DapperExtensions.IKeyPropertyResolver"/> interface by resolving key properties
        ...
        public class DefaultKeyPropertyResolver : IKeyPropertyResolver
        {
            ...
            public virtual PropertyInfo ResolveKeyProperty(Type type)
            {
                ...
                var keyProps = allProps.Where(p => p.GetCustomAttributes(true).Any(a => a is KeyAttribute || a is ExplicitKeyAttribute)).ToList(); //
        ...
        #endregion Key property resolving
        ...
        public class DefaultForeignKeyPropertyResolver : IForeignKeyPropertyResolver
        {
            ...
            private static PropertyInfo ResolveOneToOne(Type sourceType, Type includingType)
            {
                ...
                var foreignKeyProperty = sourceType.GetProperties().FirstOrDefault(p => p.Name.Equals(foreignKeyName, StringComparison.CurrentCultureIgnoreCase));
            ...
            private static PropertyInfo ResolveOneToMany(Type sourceType, Type includingType)
            {
                ...
                var foreignKeyProperty = includingType.GetProperties().FirstOrDefault(p => p.Name.Equals(foreignKeyName, StringComparison.CurrentCultureIgnoreCase));
                return foreignKeyProperty;
            }
        }

        #endregion Foreign key property resolving
        ...
        #endregion Table name resolving
        ...
        /// <summary>
        /// Implements the <see cref="DapperExtensions.IKeyPropertyResolver"/>.
        /// </summary>
        ...
        #endregion Column name resolving
